            ...
            return (<tbody  key={dt.id}>
              ...
              <td>
                <button 
                  ...
                  onClick={()=>{this.handleApprove(dt.id)}}
                  >ACCEPT</button> &nbsp;
                <button 
                  type="button" 
                  className='btn btn-danger py-0'
                  disabled={dt.status==='rejected'?true:false}
                  onClick={()=>{this.handleReject(dt.id)}}
                  >REJECT</button>
              </td>     
              </tr>
          </tbody>)})}
...
export default connect(mapStateToProps, {getAllRefunds:getAll,refundStatus:statusRefund})(AllRefund);