...
import {getAllOrders, getOrder, deliverOrder} from '../../../actions/order/index';
    ...
    return (
      <div style={{width:'100%'}}>
        ...
        <nav className="navbar navbar-light" style={{width:'100%', marginLeft:'0%'}}>
          <h4 className="navbar-brand" style={{fontSize:'24px',color:'#8f8d8d',fontFamily:'Montserrat'}}>Rica Orders</h4>
          ...
          {data.map(dt=><tbody  key={dt.id}>
              ...
              <td>
                <button 
                  ...
                  onClick={()=>{this.handleDeliver(dt.id)}}
                  disabled={(dt.status==='canceled'|| dt.status==='delivered')?true:false}>DELIVER</button>
              </td>            
              </tr>
            </tbody>)}
          </table>:<DisplayProductSkeleton/>)}
        </div>
        </div>
      </div>
    )
...
export default connect(mapStateToProps,{allOrders:getAllOrders,searchOrder:getOrder,deliverOrder:deliverOrder})(AllOrders);